...
class STRNNGraphConv(nn.Module):
    def __init__(self, args, blocks, n_vertex, adj_train, adj_val, adj_test, mode='train'):
        ...
        for l in range(len(blocks) - 3):
            modules.append(layers.STRNNBlock(args.Kt, args.Ks, n_vertex, blocks[l][-1], blocks[l+1], args.act_func, args.gso, args.enable_bias, args.droprate, edge_index, args.rnn_type, args.rnn_layers))
        ...
        if self.Ko > 1:
            self.output = layers.RNNOutputBlock(Ko, blocks[-3][-1], blocks[-2], blocks[-1][0], n_vertex, args.act_func, args.enable_bias, args.droprate,args.rnn_type, args.rnn_layers)
        elif self.Ko == 0:
